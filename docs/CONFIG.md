...
### FinalityTag
```toml
FinalityTag = false # Default
```
FinalityTag means that the chain supports the finalized block tag when querying for a block. If FinalityTag is set to true for a chain, then FinalityDepth field is ignored.
