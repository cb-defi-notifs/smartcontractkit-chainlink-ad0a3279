...
func (d *DatabaseSecrets) validateMerge(f *DatabaseSecrets) (err error) {
	if d.AllowSimplePasswords != nil && f.AllowSimplePasswords != nil {
		err = multierr.Append(err, ErrOverride{Name: "AllowSimplePasswords", Msg: "overrides are not allowed for multiple secrets files"})
	}

	if d.BackupURL != nil && f.BackupURL != nil {
		err = multierr.Append(err, ErrOverride{Name: "BackupURL", Msg: "overrides are not allowed for multiple secrets files"})
	}

	if d.URL != nil && f.URL != nil {
		err = multierr.Append(err, ErrOverride{Name: "URL", Msg: "overrides are not allowed for multiple secrets files"})
	}
...
func (e *ExplorerSecrets) validateMerge(f *ExplorerSecrets) (err error) {
	if e.AccessKey != nil && f.AccessKey != nil {
		err = multierr.Append(err, ErrOverride{Name: "AccessKey", Msg: "overrides are not allowed for multiple secrets files"})
	}

	if e.Secret != nil && f.Secret != nil {
		err = multierr.Append(err, ErrOverride{Name: "Secret", Msg: "overrides are not allowed for multiple secrets files"})
	}
...
func (p *Passwords) validateMerge(f *Passwords) (err error) {
	if p.Keystore != nil && f.Keystore != nil {
		err = multierr.Append(err, ErrOverride{Name: "Keystore", Msg: "overrides are not allowed for multiple secrets files"})
	}

	if p.VRF != nil && f.VRF != nil {
		err = multierr.Append(err, ErrOverride{Name: "VRF", Msg: "overrides are not allowed for multiple secrets files"})
	}
...
func (p *PyroscopeSecrets) validateMerge(f *PyroscopeSecrets) (err error) {
	if p.AuthToken != nil && f.AuthToken != nil {
		err = multierr.Append(err, ErrOverride{Name: "AuthToken", Msg: "overrides are not allowed for multiple secrets files"})
	}
	...
	if m.Credentials != nil && f.Credentials != nil {
		for k := range f.Credentials {
			if _, exists := m.Credentials[k]; exists {
				err = multierr.Append(err, ErrOverride{Name: fmt.Sprintf("Credentials[\"%s\"]", k), Msg: "overrides are not allowed for multiple secrets files"})
			}
		}
	}
...
func (t *ThresholdKeyShareSecrets) validateMerge(f *ThresholdKeyShareSecrets) (err error) {
	if t.ThresholdKeyShare != nil && f.ThresholdKeyShare != nil {
		err = multierr.Append(err, ErrOverride{Name: "ThresholdKeyShare", Msg: "overrides are not allowed for multiple secrets files"})
	}
